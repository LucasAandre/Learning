...
class Restaurante: # Uma classe chamada Restaurante | Nome de classe sempre com a primeira letra maiúscula, por convenção
    ...
    # def adicionar_bedida_cardapio(self, bebida):
    ...
    def adicionar_no_cardapio(self, item):
        if isinstance(item, ItemCardapio): # Se o item for uma instância (objeto) da classe ItemCardapio, faça:
    ...
    def exibir_cardapio(self):
        ...
        for i, item in enumerate(self._cardapio, start=1):
            if hasattr(item, 'descricao'): # Se tiver o atributo...
                mensagem = f'{i}. Nome: {item._nome.ljust(45)} | Preço: R${str(item._preco).ljust(25)} | Descrição: {item.descricao.ljust(25)}'
            else:
                mensagem = f'{i}. Nome: {item._nome.ljust(45)} | Preço: R${str(item._preco).ljust(25)} | Tamanho: {item.tamanho.ljust(25)}'
            print(mensagem)
